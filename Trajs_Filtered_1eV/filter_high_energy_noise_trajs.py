...
threshold = 1.0 # with respect to minimum energy configuration
...
def filter_high_energy_configs(traj_file):
    ...
    filter_fname = 'filtered_traj_1eV_' +  adsorb +  '.traj'
    write(filter_fname,filtered_traj )



    filter_energy_y = np.array(filtered_ene_list)
    save_yene_fname = 'filtered_1eV_y_' + adsorb + '.npy'
...
for adsorb in adsorb_list:
    ...
    traj_file = traj_dir + 'cleaned_overlap_'+adsorb+'.traj'
    print(traj_file)
    filter_high_energy_configs(traj_file)

    #min_ene,  max_ene, mean_ene = find_min_energy('filtered_traj_'  + adsorb +  '.traj')
    min_ene,  max_ene, mean_ene = find_min_energy('filtered_traj_1eV_'  + adsorb +  '.traj')
