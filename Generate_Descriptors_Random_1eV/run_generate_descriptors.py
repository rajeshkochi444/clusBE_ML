...
for  adsorb in adsorb_list:
    ...
    traj_file = '../Trajs_Filtered_1eV_Random/random_traj_' + adsorb + '.traj'
    print(traj_file)
    ...
    save_descriptor_fname = descriptor_type + '_ndes'+ str(n_descriptors) + '_1eV_random_' + adsorb + '.npy'    
    np.save(save_descriptor_fname, descriptor_name)
print("Completed descriptor generation")

print("Stacking descripor arrays")
for adsorb in adsorb_list:
    fname = descriptor_type + '_ndes'+ str(n_descriptors) + '_1eV_random_' + adsorb + '.npy'
    if adsorb == 'N2':
        a = np.load(fname)
        print(a.shape)
    else:
...
all_descriptors_single_npy = descriptor_type + '_ndes'+ str(n_descriptors) +  '_all_adsorb_1eV_random_' + metal_atom + '.npy'
